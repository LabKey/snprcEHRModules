                    ...
                    for (GWTPropertyDescriptor pd : packageAttributes)
                    {
                        ...
                        if (null != pd && pd.getName().equals(key))
                        {
                            ...
                            if (value == null)
                            {
                                if (pd.getLookupSchema() != null && pd.getLookupQuery() != null)
                                {
                                    logger.info("Value null for property {}, EventDataId: {}. Value skipped. Verify lookup {}.{} contains {}", pd.getName(), eventDataId, pd.getLookupSchema(), pd.getLookupQuery(), stringValue);
                                }
                                else
                                {
                                    logger.info("Value null for property {}, EventDataId: {}. Value skipped.", pd.getName(), eventDataId);
                                }
                            }
