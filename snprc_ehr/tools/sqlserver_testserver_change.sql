...
syntax changed for sql server - tjh.
*/
...
UPDATE    p
SET    Value = 'https://boomer.txbiomed.org/'
FROM	 prop.Properties p
INNER JOIN prop.PropertySets s ON s.[Set] = p.[Set] AND s.Category = 'SiteConfig'
WHERE   p.Name = 'baseServerURL'
;

UPDATE     p
SET        Value = 'Staging server'
FROM       prop.Properties p
INNER JOIN prop.PropertySets s ON s.[Set] = p.[Set] AND s.Category = 'LookAndFeel'
WHERE      p.Name = 'systemShortName'
;

UPDATE    p
SET       Value = 'EHR Staging Server'
FROM       prop.Properties p
INNER JOIN prop.PropertySets s ON s.[Set] = p.[Set] AND s.Category = 'LookAndFeel'
WHERE     p.Name = 'systemDescription'
;
...
UPDATE    ehr.module_properties
SET       stringvalue = 'boomer@txbiomed.org'
WHERE     prop_name = 'site_email'
;
...
--INSERT INTO ehr.notificationrecipients (rowid, notificationtype, container, createdby, created, modifiedby, modified, recipient) VALUES
...
--TODO: Need to disable the issue tracker for SNPRC
