...
SELECT
  ...
  case when (age."max" is NULL and age.label = 'Senior') then
    (CAST(age."min" as VARCHAR(5) ) || case when (age."min"- FLOOR(age."min", 0) > 0)  then  '' else '.0 - ' end)
  ELSE
    (CAST(age."min" as VARCHAR(5)) ||
     case when (age."min"- FLOOR(age."min", 0) > 0)  then  ' - ' else '.0 - ' end

     || CAST(age."max" as varchar(5))
     || case when (age."max"- FLOOR(age."max", 0) > 0)  then  '' else '.0' end )
  END AS ageRange,
