...
select
  ...
  CASE WHEN d.calculated_status = 'Alive' THEN
    ...
    CASE WHEN d.species.arc_species_code = 'PC' THEN
      -- Spelling matters. Consumers filter on PhyStatus <> 'Okay' (study.overdueTb), and
      -- Postgres compares text case-sensitively, so the 'OKAY' that used to be here put 30
      -- infant baboons on the overdue report. SQL Server's case-insensitive collation made
      -- it equal to 'Okay', which hid the typo. Keep this identical to the other branches.
