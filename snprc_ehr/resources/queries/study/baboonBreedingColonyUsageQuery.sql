...
FROM
  ...
  (SELECT
   ...
   WHERE aag.Id.Demographics.gender IS NOT NULL AND aag.Id.AgeClass.label IS NOT NULL AND aag.Id.Demographics.calculated_status = 'Alive'
         AND aag.Id.Demographics.species.arc_species_code = 'PC' AND aag.accountGroup in ('Colony Use', 'SNPRC Breeders')
  ...
  (SELECT
   ...
   WHERE a.isActive = true and a.assignmentStatus = 'A' AND aag.Id.Demographics.species.arc_species_code = 'PC'
         AND aag.Id.Demographics.species.arc_species_code = 'PC' and a.protocol = '278PC' AND aag.accountGroup in ('Colony Use', 'SNPRC Breeders')
