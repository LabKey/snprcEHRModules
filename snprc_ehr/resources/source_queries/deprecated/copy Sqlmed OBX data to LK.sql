...
SET @obr_date = '02/28/2023'
	...
	FROM [FROGSTAR\VORTEX].animal.dbo.CLINICAL_PATH_OBX AS obx
		INNER JOIN labkey.snprc_ehr.HL7_OBR AS obr ON obr.MESSAGE_ID = obx.MESSAGE_ID
		LEFT OUTER JOIN [FROGSTAR\VORTEX].animal.dbo.CLINICAL_PATH_LABWORK_PANELS AS lp ON obr.PROCEDURE_ID = lp.ServiceId AND obx.TEST_ID = lp.TestId
		INNER JOIN labkey.core.Containers AS c ON c.Name = 'SNPRC'

	WHERE obx.MESSAGE_ID IN (
		SELECT DISTINCT MESSAGE_ID FROM [FROGSTAR\VORTEX].animal.dbo.CLINICAL_PATH_OBX
