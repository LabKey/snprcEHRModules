...
ALTER VIEW [labkey_etl].[v_iacuc_protocols] AS
  ...
  -- 5/1/2019   use f_IACUC_num_animals_approved to get maxAnimals value, 3yr approval date, last_modification, first_approval
  -- ==========================================================================================
...
FROM dbo.arc_master AS am
  ...
  AND ad.application_status = 'A'
       ...
       LEFT JOIN dbo.v_IACUC_3yr_periods AS typ ON typ.working_iacuc = am.working_iacuc
  AND typ.[3yr_period] = (SELECT MAX([3yr_period]) FROM dbo.v_IACUC_3yr_periods WHERE working_iacuc = am.working_iacuc)

  INNER JOIN dbo.arc_detail AS ad2 ON ad2.arc_num_seq = am.arc_num_seq AND ad2.arc_num_genus = am.arc_num_genus AND
  ad2.arc_num_amendment = 0
