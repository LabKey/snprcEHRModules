...
export const setSerialOptions = (serialOptions, property, value) => {
    if (!serialOptions || !property || !value) throw new Error('setSerialOptions requires: serialOptions, property, value')

    if (constants.validSerialOptions.indexOf(property) === -1) throw new Error('Invalid setSerialOptions Property')
...
export const connect = async (port, serialOptions) => {
    if (!port || !serialOptions) throw new Error('Connection request requires port and serialOptions')
...
export const write = async (connection, text) => {
    if (!connection) throw new Error('Write requires a valid connection object')
        ...
        if (value) {
            ...
            value.forEach(v => {
                if (v >= 32 && v <= 126) // only transform printable characters
                    { data += String.fromCharCode(v) } else if (v === 13) cr = true
            })
        }
        if (cr) break
    }
