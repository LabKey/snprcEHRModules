...
import { Button, Modal, Panel, PanelGroup, Alert } from 'react-bootstrap';
...
class ProjectsView extends React.Component<any, any> {
            ...
            <div className="scheduler-view">
                ...
                >
                    <div style={{backgroundColor: 'transparent'}}>
                        <FontAwesomeIcon icon="spinner" size="9x" pulse />
                    </div>
                </Modal>

                <Confirm
                    show={confirm ? confirm.show : false}
                ...
                <AlertModal
                    show={alertModal ? alertModal.show : false}
                ...
                {alertBanner?.show && (
                    <Alert className="alert-banner" bsClass={'alert alert-' + alertBanner.variant} onDismiss={this.dismissBanner}>
                        {alertBanner.msg}
                    </Alert>
                )}

                {(!alertBanner || !alertBanner.show) && <div className="row spacer-row" />}

                {hasPermission && (
                    <div className="row">
                        <div className="col-sm-12 zero-right-padding">{this.getDetailComponent(accordion?.tab)}</div>
                            ...
                            <div className='scheduler-save-cancel'>
                                <div className='col-sm-6'>
                                    <Button disabled={this.props.selectedTimeline == null || this.props.selectedTimeline.RevisionNum == null}
                                            ...
                                            className='scheduler-save-cancel-btn'
                                    >
                                        Save
                                    </Button>
                                </div>
                                <div className='col-sm-6'>
                                    <Button onClick={this.cancel} className='scheduler-save-cancel-btn'>Cancel</Button>
                                </div>
                            </div>
                        </div>
                        <div className="col-sm-8 zero-side-padding">{this.getMainComponent(accordion?.tab)}</div>
                    </div>
                )}
            </div>
        );
